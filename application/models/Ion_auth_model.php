...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		$query = $this->db->get_where($this->tables['administrator'], ['name' => $this->config->item('default_group', 'ion_auth')], 1)->row();
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['administrator'].' as id, '.$this->tables['administrator'].'.name, '.$this->tables['administrator'].'.description')
		                ->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
		                ->join($this->tables['administrator'], $this->tables['users_groups'].'.'.$this->join['administrator'].'='.$this->tables['administrator'].'.id')
	...
	public function groups()
	{
		...
		$this->response = $this->db->get($this->tables['administrator']);
